...
const update_balance = async (data: Object_type): Promise<object> => {
    try {
        ...
        const [user_detail, merchant_balance] = await Promise.all([
            ...
            query_db(models.merchant_balance).find_one({ f4b_account_id }, {}, { take: 1, order: { id: 'DESC' } })
        ]);

        if (!user_detail) throw new Bad_request_error('User account details not found');
        if (!user_detail.bank_detail) throw new Bad_request_error('Bank account details not found for this account');
