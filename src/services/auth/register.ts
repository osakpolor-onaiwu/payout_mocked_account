...
import { merchant_bank_details
 } from '../../entities/merchant_bank_details';
...
const register = async (data: Object_type): Promise<object> => {
    try {
        ...
        if (fetch_account_id) throw new Bad_request_error('Account already exists.');
        ...
        const hash_password = await hash(get_bank_info.nuban, process.env.SALT_ROUND || 10);
